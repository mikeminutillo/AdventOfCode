...
public class Day07 : AdventOfCodeBase<Day07>
{
    ...
    IEnumerable<Instruction> Parse(string input)
        => from line in input.AsLines()
           select
            Instruction.Parse(line, @"^(\w+) -> (\w+)", i => i[0]) ??
            Instruction.Parse(line, @"^(\w+) AND (\w+) -> (\w+)", i => (ushort)(i[0] & i[1])) ??
            Instruction.Parse(line, @"^(\w+) OR (\w+) -> (\w+)", i => (ushort)(i[0] | i[1])) ??
            Instruction.Parse(line, @"^(\w+) LSHIFT (\d+) -> (\w+)", i => (ushort)(i[0] << i[1])) ??
            Instruction.Parse(line, @"^(\w+) RSHIFT (\d+) -> (\w+)", i => (ushort)(i[0] >> i[1])) ??
            Instruction.Parse(line, @"^NOT (\w+) -> (\w+)", i => (ushort)~i[0]) ??
    ...
    record Instruction(string[] Inputs, string Output, Func<ushort[], ushort> Function)
    {
        ...
        public static Instruction? Parse(string line, string pattern, Func<ushort[], ushort> function)
